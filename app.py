...
def _sort_tasks(tasks):
    """一覧の並び: 優先度（高→中→低）→ 期日（近い順）。期日なしは後ろ。"""
    def sort_key(t):
        p = PRIORITY_ORDER.get(t.get("priority") or "中", 1)
        d = t.get("due_date") or ""
        if not d:
            return (p, datetime.max)
        try:
...
def add_task():
    ...
    if not title:
        ...
        return render_template(
            "form.html",
            task={"title": title, "content": content, "due_date": due_date, "priority": priority},
            title="やることを登録",
        )

    try:
        sheets_helper.add_task(title, content, due_date, priority=priority, status="未完了")
...
def edit_task(task_id):
    ...
    if not title:
        ...
        return render_template(
            "form.html",
            task={"id": task_id, "title": title, "content": content, "due_date": due_date, "priority": priority},
            title="やることを編集",
        )

    try:
        sheets_helper.update_task(task_id, title, content, due_date, priority=priority, status=None)
...
def toggle_status(task_id):
    ...
    try:
        ...
        if task is None:
            ...
            new_status = "完了" if task.get("status") == "未完了" else "未完了"
            sheets_helper.update_status(task_id, new_status)
            flash("状態を更新しました。", "success")
    except Exception as e:
        flash(f"状態の更新に失敗しました: {e}", "error")
    return redirect(url_for("list_tasks"))
